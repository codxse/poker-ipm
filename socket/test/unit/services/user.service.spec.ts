...
describe('UserService', () => {
  ...
  describe('getPaginatedUsers', () => {
    it('should return paginated users', async () => {
      ...
      const LIMIT = 10
      userRepositoryMock.findAndCount.mockResolvedValue([users.slice(0, LIMIT), users.length])

      const paginationDto: PaginationDto = { page: 1 };
      const { data, page, totalItems } = await userService.getPaginatedUsers(paginationDto)
